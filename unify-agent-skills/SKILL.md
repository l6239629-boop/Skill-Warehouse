...
This skill also includes a local HTML manager. It starts a `127.0.0.1` server, opens the manager in the browser, lists active and disabled skills grouped by top-level project folder, records add/modify/disable/enable/delete events, keeps a per-skill version knowledge base, supports Chinese and English display, and lets the user disable or delete skills through the UI.
...
- `assets/manager/index.html`: browser UI for the SharedSkills Manager, including Chinese display and version knowledge-base views.
